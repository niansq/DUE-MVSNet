...
def get_depth_values(current_depth, n_depths, depth_interval, uncertainty_map):
    """
    ...
    constant = 1.5
    new_interval = uncertainty_map * depth_interval * constant

    '''
    new_interval[new_interval <= 1.5] = 1.5
    '''
    new_interval[new_interval <= 1.5] = 1.5

    # print(new_interval.shape)
    # print(depth_interval.shape)

    '''
    depth_min = torch.clamp_min(current_depth - n_depths / 2 * new_interval, 1e-7)
    '''

    depth_min = torch.clamp_min(current_depth - n_depths / 2 * new_interval, 1e-7)

    #depth_min = torch.clamp_min(current_depth - n_depths / 2 * new_interval, 1e-7)
    depth_values = depth_min + new_interval * \
