...
from loss_uncert import model_loss
...
class MVSSystem(LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        loss, uc_loss = self.loss(results, depths, masks)
       
        log['train/loss'] = loss
        log['train/uc_loss'] = uc_loss

        loss_uncert = model_loss(results, depths, masks)
        log['train/loss_uncert'] = loss_uncert['loss']
        # print(loss_uncert)
        loss = loss + loss_uncert['loss']
        with torch.no_grad():
            if batch_nb == 0:
                ...
                error_map_ = torch.abs(depths['level_0'][0] - results['depth_0'][0])*masks['level_0'][0].float()
                error_map = visualize_depth(error_map_)
                ...
                stack_error_map = torch.stack([error_map])                  
                self.logger.experiment.add_images('train/error_map',
                                                  stack_error_map, self.global_step)
            ...
            log['train/acc_1mm'] = acc_1mm = acc_threshold(depth_pred, depth_gt, mask, 1).mean()
        ...
        return {'loss': loss,
                'progress_bar': {'train_abs_err': abs_err, 'train_acc_1mm':acc_1mm},
                'log': log
               }
    ...
    def validation_step(self, batch, batch_nb):
        ...
        # abs_err_scaled =  abs_err/(depth_interval *192./192.)
        # epe = abs_err_scaled[mask].mean()
        # less1 = (abs_err_scaled[mask] < 1.).to(depth_pred.dtype).mean()
        # less3 = (abs_err_scaled[mask] < 3.).to(depth_pred.dtype).mean()
        # log['val_epe'] = abs_err_scaled.sum()
        # log['val_less1'] = (abs_err_scaled < 1.).to(depth_pred.dtype).sum()
        # log['val_less3'] = (abs_err_scaled < 3.).to(depth_pred.dtype).sum()

        # abs_err = (depth_gt - depth_pred).abs()
        # abs_err_scaled = abs_err/depth_interval
        # log['val_epe'] = abs_err_scaled[mask].mean().sum()
        # log['val_less1'] = (abs_err_scaled[mask] < 1.).to(depth_pred.dtype).sum()
        # log['val_less3'] = (abs_err_scaled[mask] < 3.).to(depth_pred.dtype).sum()
    ...
    def validation_epoch_end(self, outputs):
        ...
        # mean_epe = torch.stack([x['val_epe'] for x in outputs]).mean()
        # mean_less1 = torch.stack([x['val_less1'] for x in outputs]).sum()/mask_sum
        # mean_less3 = torch.stack([x['val_less3'] for x in outputs]).sum()/mask_sum
        ...
        return {'progress_bar': {'val_loss': mean_loss,
                ...
                'log': {'val/loss': mean_loss,
                        ...
                        # 'val/epe':mean_epe,
                        # 'val/less1':mean_less1,
                        # 'val/less3':mean_less3
                        }
               }
